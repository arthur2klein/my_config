...
require("lazy").setup({
  spec = {
    ...
    -- File explorer
    {
      ...
      tag = "0.1.8",
      dependencies = { "nvim-lua/plenary.nvim", "nvim-telescope/telescope-dap.nvim", "nvim-telescope/telescope-symbols.nvim" },
      config = function()
      ...
      config = function()
        ...
        local function enter_subject()
          vim.ui.input({ prompt = "Enter commit subject:" }, function(subject)
            if not subject then print("❌ Commit cancelled.") end
        ...
        local function select_commit_type()
          local commit_types = { "build", "chore", "ci", "docs", "feat", "fix", "perf", "refactor", "revert", "style",
            "test" }
          vim.ui.select(commit_types, { prompt = "Select commit type:" }, function(choice)
            if not choice then print("❌ Commit cancelled.") end
        ...
        -- Run the function
        vim.keymap.set("n", "<leader>gg", create_commit)
      end
    },
    {
      dir = "floaterminal",
