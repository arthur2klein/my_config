...
data=$(curl https://api.open-meteo.com/v1/forecast\?latitude\=43.3112\&timezone=Europe%2FParis\&longitude\=-0.3558\&current\=temperature_2m,relative_humidity_2m,is_day,wind_speed_10m,precipitation,weather_code)
current_temperature="$(echo "$data" | jq -r '.current.temperature_2m')°C"
current_humidity="$(echo "$data" | jq -r '.current.relative_humidity_2m')%"
night_icon=$([ $(echo "$data" | jq -r '.current.is_day') -eq 0 ] && echo "󰖔"|| echo " ")
...
echo "$night_icon $current_weather_code $current_temperature $current_humidity $current_precipitation $current_wind"
