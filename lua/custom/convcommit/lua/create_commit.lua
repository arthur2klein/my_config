...
local function preview()
  multiline({ prompt = "Confirm message:", default = commit_builder.build(builder) }, function(message)
    ...
    elseif status == 1 then
      print("❌ Commit received failure!")
    elseif status == 128 then
      print("❌ Commit received fatal error!")
    else
      print("❌ Commit received an other error!")
    end
  end)
end
...
function prelimirary_checks()
  ...
  if vim.fn.filereadable(vim.fn.trim(merge_state)) == 1 then
    vim.notify("Merge in progress. Please resolve conflicts before committing.", vim.log.levels.ERROR)
    return false
  end
  -- Check for staged changes
  local diff_status = vim.fn.system("git diff --cached --name-only")
  if vim.trim(diff_status) == "" then
    vim.notify("No staged changes to commit.", vim.log.levels.ERROR)
    return false
  end
  ...
  if vim.trim(user_name) == "" or vim.trim(user_email) == "" then
    vim.notify("Git user.name or user.email is not set. Please configure Git.", vim.log.levels.ERROR)
    return false
  end
  return true
end
